...
from job_collector.models import JobStatus
from job_collector.scraper import PlaywrightCareerScraper
from job_collector.service import JobScanService, job_records_to_dataframe, load_companies_from_csv
...
def main() -> None:
    ...
    _render_scan_controls(config, database, logger)
    _render_job_table(database)
...
def _render_scan_controls(config: AppConfig, database: JobDatabase, logger: logging.Logger) -> None:
    if st.button("Scan all companies", type="primary"):
        try:
            companies = load_companies_from_csv(config.companies_csv_path, logger)
        except (RuntimeError, ValueError) as exc:
            logger.warning("Could not load companies CSV: %s", exc)
            st.error(str(exc))
            return
...
def _render_job_table(database: JobDatabase) -> None:
    ...
    selected_company, selected_status, title_search = _render_filters(all_jobs)
...
def _render_filters(dataframe: pd.DataFrame) -> tuple[str, str, str]:
    company_options = ["All"] + sorted(str(company) for company in dataframe["company_name"].dropna().unique())
